...
bool areSameState(void * stateA, void * stateB){
	...
	if(a->x == b->x && a->y == b->y && a->direction == b->direction){
 ...
 * pi: process index
 */
void expandNode(AS_Node * node, AS_NodePointer * nodeList, int pi){
	State * s = (State *) node->state;
	int x = s->x;
	int y = s->y;
	
	if(x+1 < maze.sizeX && MAZE_AT(x+1,y) != M_WALL && pi == 0){
		nodeList[0] = createNode(node, x+1, y);
	}
	if(x-1 >= 0 && MAZE_AT(x-1,y) != M_WALL && pi == 1){
		nodeList[1] = createNode(node, x-1, y);
	}
	if(y-1 >= 0 && MAZE_AT(x,y-1) != M_WALL && pi == 2){
		nodeList[2] = createNode(node, x, y-1);
	}
	if(y+1 < maze.sizeY && MAZE_AT(x,y+1) != M_WALL && pi == 3){
		nodeList[3] = createNode(node, x, y+1);
	}
}
