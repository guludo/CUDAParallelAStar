		...
		while(true){
			...
			#pragma omp master
			{
				...
				// for(int i = 0; i<expansionNodesLength; i++){
					// if(expansionValidity[i] == AS_V_VALID){
				...
				for(int i = 0; i<nodeCount; i++){
					...
					while(j<size){
						if(validity[j] == AS_V_VALID && !ClosedSet_hasNode(closedSet, children[j])){
						//if(validity[j] == AS_V_VALID && !closedSetHasNode[i*config->maxNodesPerExpansion+j]){
